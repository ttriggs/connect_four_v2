...
class Game
  ...
  def get_user_answer(possible_answers)
    ...
    while answer == ""
      ...
      if possible_answers.include?(answer)
        ...
        puts "\tpossible choices are: #{possible_answers}"
      end
    end
  end
  ...
  def setup_turn_queue(player1, player2, board_obj)
    ...
    message = "Coin flip... Player #{active_player.show_player_num} wins and will take the first turn!"
    board_obj.set_message(message)
  end
